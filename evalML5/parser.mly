...
pattern:
| VARIABLE               { PatternV ($1) }
| EMPTYLIST              { PatternEmpty }
| pattern CONS pattern   { Cons ($1, $3) }
| LPAREN pattern CONS pattern RPAREN { Cons ($2, $4) }
| WILDCARD               { WildCard }

clause:
| pattern ARROW expr     { Clause1 ($1, $3) }
| pattern ARROW expr OR clause    { Clause2 ($1, $3, $5) }
...
| MATCH expr WITH clause { Match ($2, $4) }
