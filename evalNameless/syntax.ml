...
let int_of_sharp sh = int_of_string (Str.string_after sh 1)

let rec string_of_expr expr =
  match expr with
  | Num i -> string_of_int i
  | Bool b -> string_of_bool b
  | Sharp i -> "#" ^ string_of_int i
  | Op (e1, op, e2) ->
      "(" ^ string_of_expr e1 ^ string_of_op op ^ string_of_expr e2 ^ ")"
  | If (e1, e2, e3) ->
      "if " ^ string_of_expr e1 ^ " then " ^ string_of_expr e2 ^ " else "
      ^ string_of_expr e3
  | Let (e1, e2) -> "let . =" ^ string_of_expr e1 ^ " in " ^ string_of_expr e2
  | LetRec (e1, e2) ->
      "let rec . = fun .  -> " ^ string_of_expr e1 ^ " in " ^ string_of_expr e2
  | Fun e -> "(fun . -> " ^ string_of_expr e ^ ")"
