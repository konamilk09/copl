...
let infer_var env x = match env with
  Env.Empty -> raise Not_found
| Env.Env(new_env, y, v) -> if x = y then EVar1
else EVar2
...
let infer_let v1 v2 = match (v1,v2) with
  (Value.Error,_) -> ELetError1
| (_,Value.Error) -> ELetError2
| _ -> ELet

let rec g_expr env expr i = match expr with
  Num (n) -> indent i
    ^ string_of_env env ^ " |- "
    ^ string_of_int n
    ^ evalto
...
| Let (x, e1, e2, v) ->
  ...
  indent i ^ string_of_judg (Evalto(env, expr, v)) ^
  begin match rule with
      ELet ->
      " by E-Let {\n"
      ^ g_expr env e1 (i+2)
      ^ g_expr new_env e2 (i+2)
    | ELetError1 ->
      " by E-LetError1 {\n"
      ^ g_expr env e1 (i+2)
    | ELetError2 ->
...
Evalto (env, expr, v) -> g_expr env expr 0
